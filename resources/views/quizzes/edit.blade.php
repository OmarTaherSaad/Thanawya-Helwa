@extends('layouts.app')
@section('title', 'Edit Quiz ' . $quiz->identifier)
@section('content')
        ...
        <div class="row justify-content-center text-center mt-5">
            <div class="col-12 col-md-6">
                ...
                <div class="form-group" v-if="type != null">
                    ...
                    <input type="text" class="form-control" v-model="question.text" id=""
                        aria-describedby="helpId" placeholder="Question Label">
                </div>
                <div class="form-group" v-if="type != null">
                    <label>Question Marks</label>
                    <input type="text" pattern="[0-9]+" class="form-control" v-model="question.mark"
                ...
                <div class="form-group" v-if="type == 'MCQ'">
                    <input type="text" pattern="[0-9]+" class="form-control" v-model="tempRightAnswer"
                        placeholder="Right Answer Place (from 1, left to right)">
                </div>
            ...
            <div class="col-6">
                ...
                <div class="form-group m-2">
                    ...
                    <select class="form-control" v-model="maker">
                        ...
                        @foreach ($members as $key => $member)
                            <option value="{{ $key }}" @selected($quiz->maker && $quiz->maker->id == $key)>{{ $member }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group m-2">
                    ...
                    <select class="form-control" v-model="revisor">
                        ...
                        @foreach ($members as $key => $member)
                            <option value="{{ $key }}" @selected($quiz->revisor && $quiz->revisor->id == $key)>{{ $member }}</option>
                        @endforeach
                    </select>
                </div>

                <button class="btn btn-success" @click="updateQuiz()">Save Quiz</button>
            </div>
        </div>
    </div>
@endsection
@section('scripts')
    ...
    <script defer>
        ...
        window.quizApp.$data.subject = "{{ $quiz->subject }}";
        window.quizApp.$data.desc = "{{ $quiz->description }}";
    </script>
@endsection
