...
@section('content')
    ...
    <form action="{{ route('ministryExam.store') }}" enctype="multipart/form-data" method="POST" class="text-left"
        dir="ltr" onsubmit="window.document.body.classList.add('loading');">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="subject">
                        ...
                        @foreach ($subjects as $key => $s)
                            <option value="{{ $key }}" @selected($key == old('subject'))>{{ $s }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
